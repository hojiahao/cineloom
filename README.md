...
<p align="center"><img src="docs/showcase/keyboard-frames.jpg" alt="机械键盘案例的四个画面：同一把深灰键盘，“青”字铭牌与底部灯效在三个镜头和定版中保持一致" width="880"></p>
...
| 定妆图 | 当时的门控判定文案是按饮料罐写的（“必须是一个容器”“标签外不能有字”），把键盘和它自己的键帽字母误判了三次，系统保留最高分的一张继续。这个问题已修复：门控现在按创意里的产品类型判定，产品自身的印字属于产品 |
| 首帧 · 质检 | 镜头 1、2 各在第 2 候选通过（100 / 85 分）；镜头 3 的分镜写了“光标划过屏幕”，图像模型四次都没画出来，按规则保留最高分继续，成片里镜头 3 是键盘特写，没有屏幕 |
| 视频 | Wan2.2 14B，三段各 360 / 352 / 347 秒 |
| 成片 | 配音 3 句、本地生成音乐 11.0 秒、定版、调色与字幕 |
| 总计 | 2682 秒 |

</details>

咖啡（横版）和面霜（竖版）两支的第一版分别出现了画幅选错和产品被换成示例产品两个缺陷（记录见 `docs/devlog.md`），修复后正在重做，完成后补在这里。
...
- [x] 多行业案例片：饮料（竖版）与机械键盘（横版）已入“成片展示”；咖啡（横版）与面霜（竖版）修复后重做中
- [ ] 音效；配音音色与情绪的选择
- [ ] 演示视频（B 站）与“十日谈”发布
