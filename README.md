...
| 定妆图 | 第 1 张即 100 分通过；之后三个镜头都从它生成，所以罐身、色带、书法“冷”字在整支片里是同一只罐 |
| 首帧 · 质检 | 每镜头 2 个候选。镜头 1 两个候选都因“罐子未立起”被拒，改提示词重生成后 100 分；镜头 2 第 1 候选因“冰晶未出现”被拒，第 2 候选通过；镜头 3 两个候选都通过，取高分。8 次质检，1 次重生成 |
| 视频 | Wan2.2 14B，367 / 346 / 343 s |
| 成片 | 配音 3 句（3.7 / 2.9 / 2.9 s）+ 本地生成音乐 10.1 s + 定版 + 调色字幕 |
| 总计 | **2256 s**，全程 DGX Spark 本地 |

### 数码 · 机械键盘“青” · 16:9 · 16.8 s · 有声
...
| 阶段 | 这次运行发生了什么 |
|---|---|
| 脚本 · 合规 | “键盘敲击，指尖起舞 / 青轴触发，实感回响指尖 / 流畅如风，指尖连接创意”；通过 |
| 定妆图 | 当时的门控判定文案是按饮料罐写的（“必须是一个容器”“标签外不能有字”），把键盘和它自己的键帽字母误判了三次；系统保留了最高分的一张继续。**这个 bug 已修**：门控现在按创意里的产品类型判定，产品自身的印字属于产品 |
| 首帧 · 质检 | 镜头 1、2 各在第 2 候选通过（100 / 85 分）；镜头 3 的分镜写了“光标划过屏幕”，图像模型四次都没画出来，按规则保留最高分继续——成片里镜头 3 是键盘特写，没有屏幕 |
| 视频 | Wan2.2 14B，360 / 352 / 347 s |
| 成片 | 配音 3 句 + 本地生成音乐 11.0 s + 定版 + 调色字幕 |
| 总计 | **2682 s**，全程 DGX Spark 本地 |

咖啡（横版）和护肤（竖版）两支：第一版分别出现了画幅选错和产品被换成示例产品两个 bug（见 `docs/devlog.md`），修复后正在重做，完成后补在这里。
