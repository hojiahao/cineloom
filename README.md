...
**提速实验（2026 年 9 月 23–24 日，同一条气泡水创意）**

一支 15 秒成片的时间花在哪：视频三段约 1060 秒，首帧与质检约 1140 秒，其余不到 60 秒。首帧段做了四项改动，视频段没有找到不改画面的办法。

| 改动 | 结果 |
|---|---|
| 首帧按视频工作尺寸生成（1920×1080 → 1280×704） | 视频本来就在这个尺寸生成，成片不变 |
| 第二个候选只在第一个被拒时才生成 | 气泡水一次运行从 7 个候选降到 4 个 |
| 质检图缩到 1024 像素再送审 | 定妆图质检 84 → 24 秒 |
| 审第 N 镜首帧时并行生成第 N+1 镜 | 质检时间大部分藏进生图时间 |
| vLLM 睡眠模式（生视频时释放两个语言模型） | **失败，已回退**：可用内存从 47 GB 升到 104 GB，视频每段仍是 340 秒；两个语言模型解码慢了一个数量级，唤醒后还需重载权重 |
| Step3-VL 跳过推理直接判定 | **弃用**：3 秒出结果，但把合格帧判成不合格，理由是编的 |

| 运行 | 首帧与质检 | 视频三段 | 总计 |
|---|---:|---:|---:|
| 基线（9 月 22 日） | 1141 s | 1056 s | 2256 s |
| 改动后，无返工 | 524 s | 1121 s | 1842 s（−18%） |
| 改动后，镜头 3 返工两次 | 933 s | 1066 s | 2250 s |

结论：首帧段省下一半以上，但一个镜头返工两次就能抵消；视频段占六成，Wan2.2 14B 四步在 720p 下每段约 340 秒是实打实的计算。不改规格时整体只能省两成左右；要减半只能把视频降到 480p 或每段缩到 4 秒，这会改变成片，目前没有采用。
